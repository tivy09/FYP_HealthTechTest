        ...
        <form method="POST" action="{{ route("admin.departments.update", [$department->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('image_id') ? 'is-invalid' : '' }}" name="image_id" id="image_id">
                    ...
                    <option value="{{ $id }}" {{ (old('image_id') ? old('image_id') : $department->images->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="status" id="status" value="1" {{ $department->status == 1 ? 'checked' : '' }}>
