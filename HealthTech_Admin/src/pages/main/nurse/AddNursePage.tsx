...
const AddNursePage = () => {
    ...
    try {
      ...
      if (data.status === 1201) {
        ...
        if (extractedData) {
          ...
          setValue('department_id', extractedData[0].id)
        }
      }
    } catch {
      console.log('error')
  ...
  const onSubmit = async () => {
    const payload = {
      ...
      emergency_contact_phone_number: getValues(
        'emergency_contact_phone_number',
      ),
    ...
    const { data: responseData } = await nurseAPI.addNewNurse(payload)

    if (responseData.status === 1402) {
      presentToast({
  ...
  return (
    <IonPage>
      <IonHeader class="center">
        <IonToolbar>
          <BackHeader />
          <IonTitle class="ion-text-center">Add New Nurse</IonTitle>
        </IonToolbar>
      </IonHeader>
      <IonContent>
