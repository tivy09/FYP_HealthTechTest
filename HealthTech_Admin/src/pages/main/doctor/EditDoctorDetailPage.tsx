...
const EditDoctorDetailPage: React.FC<Props> = ({ match }) => {
  ...
  const onSubmit = async () => {
    ...
    const { data: responseData } = await doctorAPI.updateDoctorDetail(
      match.params.id,
      payload,
    )
    if (responseData.status === 1304) {
      presentToast({
  ...
  return (
    <IonPage>
      <IonHeader class="center">
        <IonToolbar>
          <BackHeader />
          <IonTitle class="ion-text-center">Edit Doctor Ddetail</IonTitle>
        </IonToolbar>
      </IonHeader>
      <IonContent>
...
export default EditDoctorDetailPage
